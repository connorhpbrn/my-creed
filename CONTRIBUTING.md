...
Version Open, Cloud, and CLI only for a deliberate product release.
Git tags are `open-vX.Y.Z`, `cloud-vX.Y.Z`, and `cli-vX.Y.Z`.
...
(`creed_audit_log` table) where appropriate.

For an Open migration, run `supabase db reset` from `apps/open/`. For a Cloud
migration, run `npm run db:reset --workspace creed-cloud` and
`npm run db:migrations --workspace creed-cloud`. Use an isolated local database
when an existing stack contains data you need to keep. Apply and verify each
edition you changed before pushing.
