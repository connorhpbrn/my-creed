...
- Constant-time verification derives a hidden Supabase Auth owner and issues a signed, HTTP-only, `SameSite=Lax` owner cookie so a top-level MCP OAuth GET can carry the session.
...
Open fails closed when configuration or migrations are incomplete. `public.creed_schema_version()` gives the service role one bounded readiness check, and doctor plus the app layout also require PostgREST to expose `public.commit_personal_creed`. A newer applied migration is not enough: if execute was revoked on that write RPC, setup stays closed. A valid owner cookie does not skip that check. Remaining personal writes that still need a whole-file commit (proposal review, GitHub import, version-control settings) go through `public.commit_personal_creed`.
...
`CreedState` lives in `packages/creed-core/creed-data.ts`. `CreedProvider` in `packages/creed-app/components/creed/creed-provider.tsx` owns client state and sync. A first-run Personal Creed is created with `create_owned_creed` (one Identity section) and the user enters `/file`. The Get started card is the first-run teaching UI. Open installation-owner claim remains `POST /api/open/claim` and is a different step. Personal section content, name, colour, create, duplicate, reorder, archive, restore, delete, and agent permission persist through `/api/app/sections` so the editor does not require `commit_personal_creed` for those writes.
...
Theme classes live on `html`. The static root layout cannot read the theme cookie, so marketing first paint depends on an inline blocking script (`THEME_INIT_SOURCE` in `packages/creed-app/app/layout.tsx`) that matches `public/theme-init.js`. `preinit` and `next/script` `beforeInteractive` only preload on prerendered HTML, which leaves `:root` tokens until later and flashes the opposite theme when a saved preference disagrees with the device scheme. Authenticated nonce routes ignore that inline script; they paint through `InitialThemeBoundary`. Hydration rewrites `html.className` from the server string, so `ThemeProvider` restores the stored theme in `useLayoutEffect` before the next paint.
