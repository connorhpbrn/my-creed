...
3. generates the 9-digit owner code and the encryption secret;
...
Open [http://localhost:3001](http://localhost:3001), enter the owner code shown by setup, and complete Personal onboarding.
...
Generate the 9-digit owner code and the encryption secret:

```bash
node -e "console.log(String(require('node:crypto').randomInt(0, 1e9)).padStart(9, '0'))"
...
7. Redeploy, return to `/setup`, and enter the 9-digit owner code.
...
`CREED_OWNER_SECRET` is the 9-digit owner code. Enter it at `/setup` to authorise a browser once. Store it as nine digits with no spaces or hyphens. Creed then stores a signed, HTTP-only owner cookie and a hidden Supabase session in that browser. There is no recurring login screen.
