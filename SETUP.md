...
### Personal writes are missing from the database API

The schema version can look current while PostgREST no longer exposes `commit_personal_creed`. That happens if extra SQL revoked execute on the write RPC. Re-apply the Open migrations from this repository, do not keep hosted-only grant changes that hide that function, and run `npm run doctor` again. The app stays on `/setup` until the write RPC is visible.
