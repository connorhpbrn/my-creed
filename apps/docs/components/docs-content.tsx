...
npm run dev`}</DocsCommand>
        <Steps>
          ...
          <li><p>Enter the 9-digit owner code from setup.</p></li>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p>
          Setup preserves unrelated environment values, verifies the Supabase keys before saving them, and asks before it changes the database. The installer prints the 9-digit owner code once. The encryption secret is written to <InlineCode>apps/open/.env.local</InlineCode> and is never printed. Enter the owner code in the browser at <InlineCode>/setup</InlineCode>.
        </p>
        <h3>Manual setup</h3>
...
icacls apps\\open\\.env.local /inheritance:r /grant:r "\${env:USERNAME}:RW"`}</DocsCommand>
        <p>Generate the 9-digit owner code and the encryption secret on any platform:</p>
        <DocsCommand>{`node -e "console.log(String(require('node:crypto').randomInt(0, 1e9)).padStart(9, '0'))"
    ...
    content: (
      <>
        ...
        <Steps>
          ...
          <li><p>Redeploy, return to <InlineCode>/setup</InlineCode>, and enter the 9-digit owner code.</p></li>
        </Steps>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <h3>Owner access</h3>
        <p>Enter the 9-digit owner code at <InlineCode>/setup</InlineCode>. It authorises a browser once. Rotating it invalidates existing owner cookies. Store <InlineCode>CREED_OWNER_SECRET</InlineCode> as nine digits with no spaces or hyphens. Keep the owner code and encryption secret in a password manager.</p>
