...
-- All section writers share the parent lock so structural batches also detect inserts.
create or replace function public.lock_creed_section_write()
returns trigger language plpgsql security definer set search_path = '' as $$
begin
  perform 1 from public.creeds where id = coalesce(new.creed_id, old.creed_id) for update;
  return coalesce(new, old);
end;
$$;
revoke all on function public.lock_creed_section_write() from public, anon, authenticated;
drop trigger if exists creed_sections_write_lock on public.creed_sections;
create trigger creed_sections_write_lock before insert or update or delete on public.creed_sections
for each row execute function public.lock_creed_section_write();

create or replace function public.creed_section_baseline(p_creed_id uuid)
returns jsonb language sql stable security invoker set search_path = '' as $$
  select coalesce(jsonb_object_agg(section_id, jsonb_build_object(
    'revision', revision, 'position', position, 'permission', agent_permission,
    'archived', archived_at is not null)), '{}'::jsonb)
  from public.creed_sections where creed_id = p_creed_id;
$$;
revoke all on function public.creed_section_baseline(uuid) from public, anon;
grant execute on function public.creed_section_baseline(uuid) to authenticated, service_role;

create or replace function public.commit_personal_creed(
  p_creed_id uuid, p_user_id uuid, p_expected jsonb,
  p_sections jsonb, p_proposals jsonb, p_activity jsonb,
  p_version_control jsonb, p_remove_sections text[], p_remove_proposals text[],
  p_require_approval boolean, p_review jsonb default null, p_expected_settings jsonb default null
...
declare
  ...
  updates_sql text;
  conflict_sql text;
  proposal public.creed_proposals;
  settings public.creed_version_control;
begin
  if auth.uid() is distinct from p_user_id and auth.role() <> 'service_role' then
    raise exception 'Unauthorized' using errcode = '42501';
  end if;
  perform 1 from public.creeds where id = p_creed_id and owner_user_id = p_user_id
    and type = 'personal' for update;
  if not found then raise exception 'Unauthorized' using errcode = '42501'; end if;
  if p_expected is null or public.creed_section_baseline(p_creed_id) <> p_expected then
    raise exception 'creed_write_conflict' using errcode = '40001';
  end if;

  select * into settings from public.creed_version_control where creed_id = p_creed_id for update;
  perform 1 from public.creed_tokens where creed_id = p_creed_id for update;
  if p_expected_settings is null or
    (p_expected_settings->>'requireApproval')::boolean is distinct from
      (select require_approval from public.creed_tokens where creed_id = p_creed_id) or
    coalesce(settings.repo_owner, '') is distinct from p_expected_settings->'versionControl'->>'repoOwner' or
    coalesce(settings.repo_name, '') is distinct from p_expected_settings->'versionControl'->>'repoName' or
    coalesce(settings.branch, '') is distinct from p_expected_settings->'versionControl'->>'branch' or
    settings.last_remote_sha is distinct from p_expected_settings->'versionControl'->>'lastRemoteSha' or
    settings.last_synced_content_hash is distinct from p_expected_settings->'versionControl'->>'lastSyncedContentHash' then
    raise exception 'creed_write_conflict' using errcode = '40001';
  end if;
  if p_review is not null then
    select * into proposal from public.creed_proposals where creed_id = p_creed_id
      and id = p_review->>'id' for update;
    if not found or (proposal.status <> 'pending' and not
      (proposal.status = 'stale' and p_review->>'decision' = 'dismiss')) then
      raise exception 'creed_write_conflict' using errcode = '40001';
    end if;
    if p_review->>'decision' = 'accept' and (
      proposal.draft <> p_review->'draft' or
      (proposal.base_revision is not null and proposal.draft->>'kind' <> 'new-section' and
       proposal.base_revision is distinct from (select revision from public.creed_sections
         where creed_id = p_creed_id and section_id = proposal.section_id))) then
      raise exception 'creed_write_conflict' using errcode = '40001';
    end if;
  end if;

  -- Table names are fixed; values remain parameters and identifiers are quoted.
  for batch in select * from (values
    ('creed_sections', p_sections, 'creed_id,section_id'),
    ('creed_proposals', p_proposals, 'id'),
    ('creed_activity', p_activity, 'id'),
    ('creed_version_control', jsonb_build_array(p_version_control), 'creed_id')
  ) as batches(table_name, rows, conflict_columns)
  loop
    for item in select value from jsonb_array_elements(batch.rows)
    loop
      if item->>'creed_id' is distinct from p_creed_id::text then
        raise exception 'Invalid Creed target';
      end if;
      if batch.table_name in ('creed_proposals', 'creed_activity') then
        execute format('select exists(select 1 from public.%I where id = $1 and creed_id <> $2)', batch.table_name)
          into strict conflict_sql using item->>'id', p_creed_id;
        if conflict_sql::boolean then raise exception 'Invalid record target'; end if;
      end if;
      select string_agg(format('%I', key), ', '),
        string_agg(format('%1$I = excluded.%1$I', key), ', ') filter (where key <> 'created_at')
        into columns_sql, updates_sql from jsonb_object_keys(item) key;
      conflict_sql := case when batch.table_name = 'creed_activity' then 'do nothing'
        else 'do update set ' || updates_sql end;
      execute format('insert into public.%1$I (%2$s) select %2$s from jsonb_populate_record(null::public.%1$I, $1) on conflict (%3$s) %4$s',
        batch.table_name, columns_sql, batch.conflict_columns, conflict_sql) using item;
      if batch.table_name = 'creed_sections' and
        coalesce((p_expected->(item->>'section_id')->>'revision')::int, 0) <> (item->>'revision')::int then
        insert into public.creed_section_versions (creed_id, section_id, revision, name, accent, content, actor_user_id, actor_type, agent_name, cause)
          values (p_creed_id, item->>'section_id', (item->>'revision')::int, item->>'name', item->>'accent', coalesce(item->'payload'->>'content', ''), p_user_id,
            item->>'last_edited_type', case when item->>'last_edited_type' = 'agent' then item->>'last_edited_by' else null end,
            case when p_review is null then 'manual' else 'proposal' end);
      end if;
    end loop;
  end loop;
  delete from public.creed_sections where creed_id = p_creed_id and section_id = any(p_remove_sections);
  delete from public.creed_proposals where creed_id = p_creed_id and id = any(p_remove_proposals);
  if p_review is not null and p_review->>'decision' <> 'stale' then
    delete from public.creed_proposals where creed_id = p_creed_id and id = p_review->>'id';
  end if;
  update public.creed_tokens set require_approval = p_require_approval, updated_at = now()
    where creed_id = p_creed_id and require_approval is distinct from p_require_approval;
  return public.creed_section_baseline(p_creed_id);
end;
$$;
revoke all on function public.commit_personal_creed(uuid, uuid, jsonb, jsonb, jsonb, jsonb, jsonb, text[], text[], boolean, jsonb, jsonb) from public, anon;
grant execute on function public.commit_personal_creed(uuid, uuid, jsonb, jsonb, jsonb, jsonb, jsonb, text[], text[], boolean, jsonb, jsonb) to authenticated, service_role;

-- Only the server can submit a batch, after resolving effective member permissions.
create or replace function public.commit_creed_section_batch(
  p_creed_id uuid, p_expected jsonb, p_sections jsonb, p_remove text[],
  p_order text[], p_proposal_id text, p_actor jsonb, p_activity jsonb
...
declare
  ...
  proposal public.creed_proposals;
begin
  perform 1 from public.creeds where id = p_creed_id for update;
  if not found then raise exception 'Creed not found'; end if;
  if p_expected is null or public.creed_section_baseline(p_creed_id) <> p_expected then
    raise exception 'creed_write_conflict' using errcode = '40001';
  end if;
  if p_proposal_id is not null then
    select * into proposal from public.creed_proposals where creed_id = p_creed_id and id = p_proposal_id for update;
    if not found or proposal.status <> 'pending' or proposal.draft is distinct from p_actor->'proposalDraft' or
      (proposal.base_revision is not null and proposal.draft->>'kind' <> 'new-section' and
       proposal.base_revision is distinct from (select revision from public.creed_sections where creed_id = p_creed_id and section_id = proposal.section_id)) then
      raise exception 'creed_write_conflict' using errcode = '40001';
    end if;
  end if;
  for item in select value from jsonb_array_elements(p_sections)
  loop
    if item->>'creed_id' is distinct from p_creed_id::text then raise exception 'Invalid Creed target'; end if;
    select string_agg(format('%I', key), ', '), string_agg(format('%1$I = excluded.%1$I', key), ', ')
      into columns_sql, updates_sql from jsonb_object_keys(item) key;
    execute format('insert into public.creed_sections (%1$s) select %1$s from jsonb_populate_record(null::public.creed_sections, $1) on conflict (creed_id, section_id) do update set %2$s', columns_sql, updates_sql) using item;
    insert into public.creed_section_versions (creed_id, section_id, revision, name, accent, content, actor_user_id, actor_type, agent_name, cause)
      values (p_creed_id, item->>'section_id', (item->>'revision')::int, item->>'name', item->>'accent', item->'payload'->>'content',
        (p_actor->>'userId')::uuid, p_actor->>'type', p_actor->>'agentName', p_actor->>'cause');
  end loop;
  if p_remove is not null then
    delete from public.creed_member_section_permissions where creed_id = p_creed_id and section_id = any(p_remove);
    delete from public.creed_proposals where creed_id = p_creed_id and section_id = any(p_remove) and id is distinct from p_proposal_id;
    delete from public.creed_sections where creed_id = p_creed_id and section_id = any(p_remove);
  end if;
  if p_order is not null then
    update public.creed_sections s set position = ordering.ordinality - 1, updated_at = now()
      from unnest(p_order) with ordinality as ordering(section_id, ordinality)
      where s.creed_id = p_creed_id and s.section_id = ordering.section_id;
  end if;
  if p_proposal_id is not null then
    update public.creed_proposals set status = 'accepted', updated_at = now() where creed_id = p_creed_id and id = p_proposal_id;
  end if;
  if p_activity is not null then
    if p_activity->>'creed_id' is distinct from p_creed_id::text then raise exception 'Invalid activity target'; end if;
    select string_agg(format('%I', key), ', ') into columns_sql from jsonb_object_keys(p_activity) key;
    execute format('insert into public.creed_activity (%1$s) select %1$s from jsonb_populate_record(null::public.creed_activity, $1)', columns_sql) using p_activity;
  end if;
end;
$$;
revoke all on function public.commit_creed_section_batch(uuid, jsonb, jsonb, text[], text[], text, jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.commit_creed_section_batch(uuid, jsonb, jsonb, text[], text[], text, jsonb, jsonb) to service_role;
