...
// Split-screen sign-in / create-account surface shared by /signin and /signup,
      ...
      >
        {confirmation ? (
          ...
          <div className="flex h-[52px] items-center justify-center">
            <AnimatedPageTitle
              text={t.heading}
              className="whitespace-nowrap text-center text-[2rem]! md:text-[2.65rem]!"
            />
          </div>

          {privateAccess && accessDenied ? (
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-3">
            ...
            {isSignup ? (
              <label className="flex cursor-pointer select-none items-start gap-2.5 text-[13px] leading-snug text-[var(--creed-text-secondary)]">
            ...
            <AuthSubmitButton
              ...
              disabled={
                busy ||
                !configured ||
                (isSignup ? !agreeTerms : !email.trim() || !password)
              }
              login={mode === "login"}
            />
          </form>
          ...
          <p
            className="mt-5 text-center text-[14px] text-[var(--creed-text-tertiary)]"
          >
            {t.switchPrompt}{" "}
            <Link
...
function ProviderButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="relative inline-flex h-11 w-full items-center justify-center gap-2.5 rounded-xl border border-[var(--creed-border)] bg-[var(--creed-surface)] px-4 text-[15px] font-medium text-[var(--creed-text-primary)] transition-colors hover:bg-[var(--creed-surface-raised)] disabled:cursor-not-allowed disabled:opacity-50"
    >
      {lastUsed ? (
