import type { TableInsert } from "@creed/persistence/supabase/types";
import "server-only";
...
export async function updateSharedVersionControlSync(
  ...
  const row: TableInsert<"creed_version_control"> = { creed_id: creedId, updated_at: new Date().toISOString() };
