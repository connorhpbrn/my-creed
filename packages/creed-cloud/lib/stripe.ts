...
export async function syncSubscriptionFromStripe(
  ...
  if (row.stripe_subscription_id !== subscription.id) return false;

  // Event delivery is unordered. Reconcile the currently recorded subscription.
  const current = await getStripeClient().subscriptions.retrieve(subscription.id);
  const status = mapStripeSubStatus(current.status);
  const { error } = await admin
    .from("creed_entitlements")
    .update({
      status,
      current_period_end: readPeriodEnd(current),
      cancel_at_period_end: Boolean(current.cancel_at_period_end),
      billing_interval: readInterval(current),
      updated_at: new Date().toISOString(),
    })
