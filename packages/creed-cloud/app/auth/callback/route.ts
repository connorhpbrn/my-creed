...
export async function GET(request: Request) {
  ...
  // like a fresh sign-in or bounce them to /signin on any hiccup.
  ...
  if (code) {
    ...
    if (user && !canAccessCloud(user.email)) {
      ...
      return NextResponse.redirect(`${origin}/signin?access=private`);
    }
  ...
  // Only a genuinely failed sign-in/confirmation goes to /signin; link flows
  // always return to where they came from (the user is still signed in).
  const target = code && exchangeFailed && !isLinkFlow ? "/signin" : safeNext;
