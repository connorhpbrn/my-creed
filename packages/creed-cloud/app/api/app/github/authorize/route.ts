...
export async function GET(request: Request) {
  ...
  if (auth instanceof NextResponse) {
    return NextResponse.redirect(
      new URL("/signin?next=/settings", origin),
    );
  }
  ...
  if (mode === "personal") {
    creedId = url.searchParams.get("creedId")?.trim();
    if (!creedId || !await getOwnedPersonalCreedId(auth.supabase, auth.user.id, creedId)) {
      return backToSettings(origin, param, "invalid");
    }
  }
