...
test("sync status keeps structural failures distinct from local drafts", () => {
  ...
  assert.match(file, /function SaveStatus\(\) \{[\s\S]*?w-max/);
  assert.match(file, /<SwapLabel value=\{content\.label\} options=\{statusLabels\} \/>/);
  assert.match(file, /relative grid h-4 w-4[\s\S]*absolute inset-0/);
  assert.doesNotMatch(file, /key=\{displayStatus\}[\s\S]*content\.label/);
});
