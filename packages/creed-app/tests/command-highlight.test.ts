...
test("JSON numbers use the number token, not the binary token", () => {
  const tokens = typed(`{ "n": 3 }`);
  assert.ok(tokens.some((token) => token.type === "hljs-number" && token.text === "3"));
});
...
test("setup and docs share the Open setup command chip", () => {
  ...
  assert.match(source("../app/globals.css"), /--creed-command-keyword: #2563eb/);
  assert.match(source("../app/globals.css"), /--creed-command-plain: var\(--creed-text-primary\)/);
  assert.match(source("../app/globals.css"), /--creed-command-string: #22c55e/);
  assert.match(source("../app/globals.css"), /--creed-command-punct: #9333ea/);
  assert.match(source("../app/globals.css"), /--creed-command-flag: #ea580c/);
  assert.doesNotMatch(source("../app/globals.css"), /creed-command-bright/);
  assert.match(bench, /if \(!BENCH_PAGE_PUBLIC\) notFound\(\)/);
  assert.match(bench, /CreedBenchChart/);
  assert.doesNotMatch(bench, /How Creed Bench works/);
  assert.doesNotMatch(bench, /CodeCommand/);
});
