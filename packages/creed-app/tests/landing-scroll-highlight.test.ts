...
test("homepage drops the demo and first two sections for a scroll statement", () => {
  ...
  const airplane = source("../../creed-ui/airplane.tsx");
  assert.match(airplane, /animate=\{controls\}[\s\S]{0,80}initial="normal"/);
  ...
  assert.doesNotMatch(directEditDemo, /"- /);
...
test("legal contact links stay visible and inline with their copy", () => {
  const terms = source("../components/marketing/terms-page-view.tsx");
  const privacy = source("../components/marketing/privacy-page-view.tsx");

  assert.match(terms, /href=\{`mailto:\$\{section\.note\}`\}/);
  assert.match(terms, /text-\[var\(--creed-accent\)\]/);
  assert.match(privacy, /showIcoGuidance/);
  assert.match(privacy, /<PrivacyExternalLink[\s\S]{0,120}label="ICO guidance"/);
  assert.doesNotMatch(privacy, /md:justify-between/);
});
