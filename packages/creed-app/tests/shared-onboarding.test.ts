...
test("Bonus credits render only when a renewable allowance exists", async () => {
  ...
  assert.match(account, /bonusBalanceLoading \|\| bonusResets/);
  assert.match(account, /\{showBonusCard \? \(/);
});
