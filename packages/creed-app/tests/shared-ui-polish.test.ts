...
test("shared overlays, consent spacing, and connection actions stay responsive", () => {
  ...
  assert.match(picker, /DROPDOWN_AVATAR_ITEM_CLASS/);
  assert.match(toaster, /className="!z-40/);
  assert.match(connections, /value=\{setupOpen \? "Hide instructions" : "Show instructions"\}/);
  assert.match(connections, /SwapLabel/);
...
test("stacked chart bars stay put and tooltip swatches are squircles", () => {
  const bars = source("../components/creed/rounded-bar.tsx");
  const health = source("../components/creed/mcp-health-dashboard.tsx");
  const connections = source("../components/creed/connections-screen.tsx");
  const chart = source("../../creed-ui/chart.tsx");
  const quality = source("../components/creed/file-quality-ui.tsx");
  const menu = source("../../creed-ui/dropdown-menu.tsx");
  const swapLabel = source("../components/creed/swap-label.tsx");

  assert.match(bars, /isAnimationActive=\{false\}/);
  assert.match(bars, /activeBar=\{false\}/);
  assert.match(bars, /background=\{false\}/);
  assert.match(bars, /const BAR_MORPH/);
  assert.match(bars, /<motion.rect/);
  assert.match(bars, /<motion.path/);
  assert.match(health, /Keep the last chart up while a new range loads/);
  assert.match(health, /loading && !summary/);
  assert.match(health, /StackedRoundedBar/);
  assert.doesNotMatch(health, /<Bar[\s\n]/);
  assert.match(health, /function ChartEmpty/);
  assert.match(health, /<SwapLabel/);
  assert.match(health, /options=\{items\.map\(\(item\) => item\.label\)\}/);
  assert.match(health, /ChartColumnIcon/);
  assert.match(health, /function ChartStage/);
  assert.match(health, /chartHeight=\{240\}/);
  assert.match(health, /chartHeight=\{200\}/);
  assert.match(health, /contain: "layout paint"/);
  assert.match(health, /useLayoutEffect/);
  assert.match(health, /data-connections-scroll/);
  assert.match(chart, /chartHeight/);
  assert.match(chart, /width=\{chartWidth\}/);
  assert.match(chart, /height=\{chartHeight\}/);
  assert.doesNotMatch(health, /h-full min-h-\[180px\]/);
  assert.match(connections, /data-connections-scroll/);
  assert.match(chart, /rounded-\[4px\]/);
  assert.doesNotMatch(chart, /rounded-\[2px\]/);
  assert.doesNotMatch(chart, /cursor = false/);
  assert.doesNotMatch(chart, /isAnimationActive = false/);
  assert.match(chart, /itemConfig\?\.label \|\| item\.name/);
  assert.match(
    chart,
    /<span className="text-\[var\(--creed-text-primary\)\]">\s*\{itemConfig\?\.label \|\| item\.name\}/,
  );
  assert.match(quality, /DROPDOWN_CHEVRON_CLASS/);
  assert.match(menu, /w-max min-w-\[max\(8rem,var\(--radix-dropdown-menu-trigger-width\)\)\]/);
  assert.doesNotMatch(menu, /w-\(--radix-dropdown-menu-trigger-width\) min-w-32 /);
  assert.match(swapLabel, /measuredKeyRef/);
  assert.match(swapLabel, /must keep the 300ms ease/);
  assert.match(swapLabel, /WIDTH_PAD/);
  assert.match(swapLabel, /widthCache/);
  assert.match(swapLabel, /animateWidth/);
  assert.match(swapLabel, /page load and hidden-surface measures never slide/);
  const column = source("../../creed-ui/chart-column.tsx");
  assert.match(column, /pathLength: \[1, 0, 1\]/);
  assert.match(column, /duration: 0\.95/);
  assert.match(quality, /group-aria-expanded:rotate-180/);
});
...
test("Open model usage omits the redundant BYOK badge", () => {
  ...
  assert.match(settings, /value=\{aiSettings\.aiMode === "credits" \? "Credits" : "BYOK"\}/);
  assert.match(swapLabel, /useLayoutEffect/);
  assert.match(
    swapLabel,
  ...
  assert.match(
    settings,
    /USAGE_RANGE_ITEMS[\s\S]*?key: "all"[\s\S]*?key: "90d"[\s\S]*?key: "7d"/,
  );
  const health = source("../components/creed/mcp-health-dashboard.tsx");
  assert.match(
    health,
    /RANGES[\s\S]*?value: "all"[\s\S]*?value: "90d"[\s\S]*?value: "7d"/,
  );
});
...
test("file section preview reuses live heading, column, and tag chips", () => {
  ...
  assert.match(presentation, /export function FileSectionReadOnly/);
  assert.match(presentation, /export function fileSectionTagTargets/);
  ...
  assert.match(presentation, /sticky top-0 bg-\[var\(--creed-surface\)\]/);
  assert.doesNotMatch(presentation, /--creed-surface\)\]\/95|backdrop-blur-sm/);
  ...
  assert.doesNotMatch(loading, /\/95|backdrop-blur-sm/);
  ...
  assert.match(switcher, /<ProfileAvatar/);
  assert.match(switcher, /size="sm"/);
});

test("activity cards keep expand padding inside the animated body", () => {
  const fileScreen = source("../components/creed/file-screen.tsx");
  assert.doesNotMatch(fileScreen, /-mb-3 overflow-hidden/);
  assert.match(
    fileScreen,
    /creed-diff-block -mx-3 max-h-72 overflow-y-auto pt-2\.5 leading-\[1\.6\]/,
  );
});
...
test("section copy stays open with feedback and structural proposals use semantic badges", () => {
  ...
  assert.match(fileScreen, /value=\{sectionCopied \? "Copied" : "Copy"\}/);
  assert.match(fileScreen, /SwapLabel/);
...
test("Open owner unlock is only a masked code and auto-submits", () => {
  ...
  assert.match(claim, /<SwapLabel/);
  assert.match(claim, /DelayedNavigationSpinner pending=\{submitting\}/);
  assert.doesNotMatch(claim, /Opening your Creed/);
  assert.match(claim, /nextPath = "\/file"/);
  assert.doesNotMatch(claim, /LoaderCircle/);
...
test("account menu rows match the switcher inset and row padding", () => {
  ...
  assert.match(accountMenu, /h-8 gap-1\.5 rounded-sm px-2\.5 py-0/);
  ...
  assert.match(feedback, /md:w-\[340px\]/);
  assert.doesNotMatch(feedback, /md:w-\[384px\]/);
...
test("proposal navigation keeps existing section headings below the sticky header", () => {
  const file = source("../components/creed/file-screen.tsx");
  const revealStart = file.indexOf("function proposalReveal");
  const revealEnd = file.indexOf("// Per-section fingerprints", revealStart);
  const proposalReveal = file.slice(revealStart, revealEnd);

  assert.match(proposalReveal, /return sectionReveal\(proposal\.sectionId\)/);
  assert.doesNotMatch(proposalReveal, /delete-section/);
  assert.match(
    file,
    /stickyOffset[\s\S]*elementRect\.top[\s\S]*stickyOffset\s*-\s*16/,
  );
});
...
test("dropdown menus share the switcher card inset, icon row pad, and flipping chevrons", () => {
  const menu = source("../../creed-ui/dropdown-menu.tsx");
  const switcher = source("../components/creed/creed-switcher.tsx");
  ...
  assert.match(menu, /bg-\[var\(--creed-surface\)\] p-1"/);
  assert.match(menu, /export const DROPDOWN_AVATAR_ITEM_CLASS/);
  assert.match(menu, /flex h-8 cursor-default items-center gap-1\.5 rounded-sm px-2\.5 py-0 text-\[13px\]/);
  ...
  assert.match(switcher, /DROPDOWN_AVATAR_ITEM_CLASS/);
  assert.match(switcher, /DROPDOWN_CHEVRON_CLASS/);
  assert.match(switcher, /text-\[var\(--creed-text-primary\)\]!/);
...
test("mobile review-pill actions sit 2px closer to the card edges", () => {
  ...
  const desktop = reviewPill.slice(
    reviewPill.indexOf("DropdownMenuSubTrigger"),
    reviewPill.indexOf("function ReviewPillItem"),
  );
  assert.match(desktop, /SEP_DOT_CLASS/);
  assert.match(desktop, /inline-flex shrink-0 items-center gap-1/);
  assert.match(desktop, /ml-auto rotate-90 shrink-0/);
  const item = reviewPill.slice(reviewPill.indexOf("function ReviewPillItem"));
  assert.doesNotMatch(item, /proposed/);
  assert.match(item, /item\.sectionName \?\? item\.proposal\.sectionName/);
  assert.match(item, /DiffBadge tone="removed"[\s\S]{0,280}<AnimatedChevronDown/);
  assert.match(
    item,
    /ml-auto shrink-0 origin-center[\s\S]*duration-200 ease-\[cubic-bezier\(0\.22,1,0\.36,1\)\]/,
  );
  assert.match(item, /in-aria-expanded:rotate-0/);
  assert.match(item, /SEP_DOT_CLASS/);
  assert.doesNotMatch(
    item.slice(0, item.indexOf("AnimatePresence")),
    /creed-success|creed-danger/,
  );
  assert.match(reviewPill, /flex flex-col gap-0\.5 md:w-\[280px\]/);
  assert.match(reviewPill, /ResizeObserver/);
  assert.match(reviewPill, /isMobile && menuLayout/);
  assert.match(reviewPill, /width: menuLayout\.width/);
  assert.match(reviewPill, /querySelector<HTMLElement>/);
  assert.doesNotMatch(reviewPill, /w-\[min\(100vw-48px,300px\)\]/);
  assert.match(
    reviewPill,
    /pointer-events-none select-none !text-\[var\(--creed-text-tertiary\)\]/,
  );
  assert.equal([...reviewPill.matchAll(/SEP_DOT_CLASS/g)].length, 4);
});
