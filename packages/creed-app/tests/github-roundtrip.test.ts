...
test("bare and angle URLs become links", () => {
  ...
  const bareHost = markdownToRichHtml("See [docs](hello.com).");
  assert.match(bareHost, /<a href="https:\/\/hello.com"/);
  const slashedHost = markdownToRichHtml("See [docs](/hello.com).");
  assert.match(slashedHost, /<a href="https:\/\/hello.com"/);
});
