...
test("stale proposals leave inline and pending review controls", () => {
  const proposals = [
...
test("stale proposals remain discoverable when their activity row is missing", () => {
  const stale = proposal("old", "stale");
  const entries = proposalActivityEntries([], [stale, proposal("pending", "pending")]);
  assert.equal(entries.length, 1);
  assert.equal(entries[0].proposalId, stale.id);
  assert.equal(entries[0].status, "stale");
  assert.equal(entries[0].afterText, "<p>old</p>");
  assert.equal(entries[0].beforeText, undefined);
});

test("stale activity reconciles pending rows without losing snapshots or duplicating history", () => {
  const stale = proposal("old", "stale");
  const [resolved] = proposalActivityEntries([], [stale]);
  const pending = { ...resolved, id: "pending-row", status: "pending" as const, beforeText: "original" };
  const entries = proposalActivityEntries([pending], [stale]);
  assert.equal(entries.length, 1);
  assert.equal(entries[0].status, "stale");
  assert.equal(entries[0].beforeText, "original");
  assert.deepEqual(proposalActivityEntries([pending, resolved], [stale]), [resolved]);
});
