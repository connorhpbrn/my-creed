...
test("Open owner access is cryptographically checked and gates pages and APIs", () => {
  ...
  assert.match(ownerCore, /OPEN_OWNER_CODE_LENGTH = 9/);
  assert.match(read("packages/creed-app/components/auth/open-owner-claim-form.tsx"), /OWNER_CODE_LENGTH = 9/);
...
test("authorised Open owners stay on /setup until schema is ready", () => {
  ...
  assert.match(setupPage, /redirect\("\/"\)/);
  assert.match(setupPage, /OpenOwnerClaimForm/);
  ...
  assert.match(onboarding, /redirect\("\/setup\?next=\/onboarding"\)/);
  assert.match(onboarding, /result\.state\.sections\.length === 0/);
  assert.match(read("apps/open/edition/config.ts"), /unauthenticated: "\/setup"/);
  assert.match(read("apps/open/proxy.ts"), /unauthenticatedRoot: "\/setup"/);
  assert.match(nextConfig, /source: "\/claim"/);
  assert.match(nextConfig, /destination: "\/setup"/);
});
...
test("Open accepts Vercel's system origin before user configuration", () => {
  ...
  assert.match(setupPage, /isSiteUrlConfigured\(\)/);
});
