...
test("Open owner access is cryptographically checked and gates pages and APIs", () => {
  ...
  assert.match(owner, /sameSite: "lax"/);
  assert.match(owner, /CREED_OWNER_SECRET/);
  const authorize = read("packages/creed-open/app/authorize/page.tsx");
  assert.doesNotMatch(authorize, /Authorise this browser/);
  assert.match(
    authorize,
    /redirect\(withOpenNextQuery\(OPEN_ENTER_PATH, returnTo\)\)/,
  );
  assert.match(authorize, /unstable_rethrow\(error\)/);
  ...
  assert.match(authorize, /getRequestAuth/);
...
test("Open setup uses a versioned readiness RPC and deterministic owner record", () => {
  ...
  assert.match(setup, /write-rpc-missing/);
  assert.match(setup, /openApiExposesCommitPersonalCreed/);
...
test("Open content tables have membership SELECT policies", () => {
  ...
  assert.match(baseline, /commit_personal_creed/);
  assert.match(
    baseline,
    /grant execute on function public\.commit_personal_creed\([^)]+\) to authenticated, service_role/,
  );
  const lastRevoke = baseline.lastIndexOf("revoke all on function public.commit_personal_creed");
  const lastGrant = baseline.lastIndexOf("grant execute on function public.commit_personal_creed");
  assert.ok(lastGrant > lastRevoke);
