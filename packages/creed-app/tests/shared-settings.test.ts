...
test("archived sections reuse the live file read-only renderer", () => {
  for (const source of [settings, sharedSettings]) {
    assert.match(source, /fileSectionTagTargets\(state\.sections\)/);
  }
  const archived = settings.slice(settings.indexOf('id="settings-archived"'));
  const sharedArchived = sharedSettings.slice(
    sharedSettings.indexOf('title="Archived"'),
  );
  for (const source of [archived, sharedArchived]) {
    assert.match(source, /<FileSectionReadOnly/);
    assert.match(source, /sectionTagTargets=\{sectionTagTargets\}/);
    assert.doesNotMatch(source, /hidden overflow-hidden md:block/);
