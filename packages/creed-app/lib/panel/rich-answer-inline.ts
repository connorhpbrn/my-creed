...
import { coerceRichTextHref } from "@creed/core/rich-text";
...
export function parseInlineMarkdown(text: string): InlineNode[] {
  ...
  const expand = (value: string): InlineNode[] => {
    ...
    for (const match of value.matchAll(PLACEHOLDER)) {
      ...
      if (start > last) nodes.push({ type: "text", text: value.slice(last, start) });
  ...
  const applyEm = (value: string) =>
    value.replace(
      /(^|[^*_])(?:\*|_)([^*_\n]+?)(?:\*|_)(?!\*|_)/g,
      (_match, lead: string, inner: string) => `${lead}${put({ type: "em", children: expand(inner) })}`,
    );
  ...
  next = next.replace(
    ...
    (_match, lead: string, name: string) => `${lead}${put({ type: "tag", name })}`,
  );
  next = next.replace(
    /\[([^\]\n]+?)\]\(([^)\s]+?)\)/g,
    (match, label: string, href: string) => {
      ...
      return put({ type: "link", href: safe, children: expand(applyStrong(label)) });
    },
  );
  next = next.replace(
    /(^|\s)(https?:\/\/[^\s<]+)/gi,
