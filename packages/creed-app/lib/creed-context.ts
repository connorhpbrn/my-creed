...
export const resolveActiveCreed = cache(async function resolveActiveCreed(
  ...
  const explicit = (await headers()).get("x-creed-id");
  if (explicit && !creeds.some((creed) => creed.id === explicit)) return null;
  const requested = explicit ?? cookieStore.get(ACTIVE_CREED_COOKIE)?.value ?? null;
...
export async function resolveManagedSharedCreedId(
  ...
  if (requestedId) {
    const creed = await resolveMemberCreedById(client, user, requestedId);
    return creed?.type === "shared" && (creed.role === "owner" || creed.role === "admin") ? creed.id : null;
  }
  const active = await resolveActiveCreed(client, user);
  if (!active) return null;
  const type = active.creeds.find((c) => c.id === active.creedId)?.type;
