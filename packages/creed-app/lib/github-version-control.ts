...
export async function requireAuthenticatedGitHubAccess() {
  const { supabase, user, creedId, creedType } = await requireAuthenticatedUser();
  if (creedType !== "personal") throw new Error("Personal GitHub access is not available for this Creed.");

  const [enrichedUser, initialIntegration, versionControl] = await Promise.all([
    enrichAuthenticatedUser(user),
    readGitHubIntegration(supabase, user.id, creedId),
    readVersionControlConfig(supabase, user.id, creedId),
  ]);
  let integration = initialIntegration;
  if (integration) {
    integration = await refreshGitHubIntegrationIfPossible(supabase, user.id, integration, false, creedId);
  }
...
export async function requireAuthenticatedUser() {
  ...
  const requestedId = (await headers()).get("x-creed-id");
  const active = requestedId ? null : await resolveActiveCreed(auth.supabase, auth.user);
  const creedId = requestedId ?? active?.creedId;
  const creed = creedId ? await resolveMemberCreedById(auth.supabase, auth.user, creedId) : null;
  if (!creed || (creed.role !== "owner" && creed.role !== "admin")) {
    throw new Error("Unauthorized");
  }
  return { supabase: auth.supabase, user: enrichedUser, creedId: creed.id, creedType: creed.type };
}
