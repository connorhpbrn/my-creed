...
async function applyDraft(params: {
  ...
  const sectionId = params.resolvedSectionId ?? params.sectionId ?? `section-${randomBytes(8).toString("hex")}`;
  ...
  if (error?.message.includes("creed_write_conflict") && params.retryWriteConflict !== false) {
    return applyDraft({ ...params, resolvedSectionId: sectionId, retryWriteConflict: false });
  }
...
export async function setSharedSectionPermission(params: {
  creedId: string;
  user: User;
  sectionId: string;
  agentPermission: AgentPermission;
}): Promise<SectionWriteResult> {
  const { creedId, user, sectionId, agentPermission } = params;
  const db = admin();
  ...
  if (!canManageSectionsLifecycle(role)) {
    return {
      ok: false,
      code: "forbidden",
      error: "Only the owner or an admin can change section permissions.",
    };
  }
  ...
  if (!current) return { ok: false, code: "not_found", error: "Section not found." };
  if (current.agent_permission === agentPermission) {
    return { ok: true, revision: current.revision };
  }
  ...
  const { error } = await db
    ...
    .update({
      ...
      agent_writable: permissionToWritable(agentPermission),
      payload: {
        ...current.payload,
        agentPermission,
...
type PersonalProposalReviewParams = {
  creedId: string;
  user: User;
  proposalId: string;
...
export async function reviewPersonalProposal(
  params: PersonalProposalReviewParams,
): Promise<SectionWriteResult> {
  return reviewPersonalProposalAttempt(params, true);
}

async function reviewPersonalProposalAttempt(
  params: PersonalProposalReviewParams,
  retryWriteConflict: boolean,
): Promise<SectionWriteResult> {
  ...
  try {
    ...
    if (error instanceof PersistenceConflict && retryWriteConflict) {
      return reviewPersonalProposalAttempt(params, false);
    }
