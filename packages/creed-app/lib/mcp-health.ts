...
export const MCP_HEALTH_RANGES: readonly McpHealthRange[] = [
  "7d",
  "30d",
  "90d",
  "all",
...
function rangeDays(range: Exclude<McpHealthRange, "all">) {
...
function buildDayWindow(range: Exclude<McpHealthRange, "all">): string[] {
  const days: string[] = [];
...
// All-time charts only plot days that have data, so a two-year account does
// not grow a 700-bar empty axis.
function buildAllTimeDayWindow(
  readRows: ReadEventRow[],
  activityRows: ActivityRow[],
): string[] {
  const days = new Set<string>();
  for (const row of readRows) days.add(row.day.slice(0, 10));
  for (const row of activityRows) days.add(row.created_at.slice(0, 10));
  if (days.size === 0) return [isoDay(new Date())];
  return [...days].sort();
}
...
export async function loadMcpHealth(
  ...
  const fetchStart =
    range === "all" ? "1970-01-01" : buildDayWindow(range)[0];
  const fetchStartIso = `${fetchStart}T00:00:00.000Z`;
  ...
  const [readEvents, clients, activity] = await Promise.all([
    db.from("creed_mcp_read_events").select("client_id, day, read_count").eq(scopeColumn, scopeValue).gte("day", fetchStart),
    ...
    db.from("creed_activity").select("actor, actor_type, section_id, section_name, accent, status, created_at").eq(scopeColumn, scopeValue).gte("created_at", fetchStartIso),
  ]);
  ...
  const window =
    range === "all"
      ? buildAllTimeDayWindow(readRows, activityRows)
      : buildDayWindow(range);
  const windowStartIso = `${window[0]}T00:00:00.000Z`;
