import type { CreedPersistenceState } from "@creed/core/validation/creed-state";
...
export async function readGitHubIntegration(client: unknown, userId: string, creedId?: string) {
  return readGithubIntegrationRow(client as SupabaseLikeClient, userId, creedId);
}
...
export async function readVersionControlConfig(
  ...
  return readVersionControlRow(client as SupabaseLikeClient, userId, creedId);
}
...
export async function clearGitHubIntegration(client: unknown, userId: string, targetCreedId?: string) {
  const db = client as SupabaseLikeClient;
  const creedId = targetCreedId
    ? await getOwnedPersonalCreedId(db, userId, targetCreedId)
    : await getPersonalCreedId(db, userId);
  ...
  // Keep the integration row at disconnected so Settings can distinguish a
  // dropped GitHub connection from one that was never connected.
...
async function loadCreedStateImpl(
  ...
  return {
    state: {
      ...baseState,
      creedId: personalCreedId,
      persistenceBaseline: sectionBaseline((sectionRows as SectionRow[] | null) ?? []),
      persistenceSettingsBaseline: { requireApproval: tokenRow.require_approval, versionControl: buildVersionControlSettings(versionControl) },
      lastSavedAt,
      readUrl: buildReadUrl(readToken),
      readToken,
      writeToken: proposalToken,
...
export async function persistCreedState(
  client: unknown,
  userId: string,
  state: CreedPersistenceState,
  targetCreedId?: string,
  review?: { id: string; decision: string; draft: unknown },
) {
  const db = client as SupabaseLikeClient;
  const requestedCreedId = targetCreedId ?? state.creedId;
  if (!requestedCreedId || (state.creedId && state.creedId !== requestedCreedId)) {
    throw new Error("An explicit matching Creed is required.");
  }
  const creedId = await getOwnedPersonalCreedId(db, userId, requestedCreedId);
  if (!creedId) {
  ...
  if (!state.persistenceBaseline) throw new PersistenceConflict();
  const { data, error } = await db.rpc("commit_personal_creed", {
    p_creed_id: creedId,
    p_user_id: userId,
    p_expected: state.persistenceBaseline,
    p_sections: sectionRows,
    p_proposals: proposalRows,
    p_activity: activityRows,
    p_version_control: versionControlRow,
    p_remove_sections: removableSectionIds,
    p_remove_proposals: removableProposalIds,
    p_require_approval: state.settings.requireApproval,
    p_review: databaseJson(review ?? null),
    p_expected_settings: state.persistenceSettingsBaseline ?? null,
  });
  if (error?.message.includes("creed_write_conflict")) throw new PersistenceConflict();
  if (
    error?.message.includes("schema cache") ||
    error?.message.includes("Could not find the function")
  ) {
    throw new Error(
      "Could not save Creed. Personal writes are missing from the database API.",
    );
  }
  assertNoError(error, "Could not save Creed.");
  return data as CreedState["persistenceBaseline"];
}
...
> {
  const db = client as SupabaseLikeClient;
  const personalCreedId = await getOwnedPersonalCreedId(db, userId, params.creedId);
  ...
  if (params.accent !== undefined && !isSelectableAccentKey(params.accent)) {
    return { ok: false, code: "failed", error: "That colour is not available." };
  }

  const contentChanged =
    params.content !== undefined &&
    !richTextContentEquivalent(current.payload.content ?? "", params.content);
  const nameChanged = params.name !== undefined && params.name !== current.name;
  const accentChanged =
    params.accent !== undefined && params.accent !== current.accent;
...
// Personal creates used to ride the full-file commit RPC. That path can fail
// while per-section PUTs still succeed, so new sections insert through RLS.
export async function createPersonalSection(
  ...
  | {
      ok: false;
      code: "forbidden" | "not_found" | "exists" | "failed";
      error: string;
    }
> {
  const db = client as SupabaseLikeClient;
  ...
  if (personalCreedId !== params.creedId) {
    return {
      ok: false,
      code: "forbidden",
      error: "You cannot edit this Creed.",
    };
  }

  const name = params.name.trim();
  if (!name) {
    return { ok: false, code: "failed", error: "A section needs a name." };
  }
  const accent =
    params.accent && isSelectableAccentKey(params.accent)
      ? params.accent
      : "stack";
  const sectionId =
    params.sectionId?.trim() || `section-${Date.now()}-${randomBytes(4).toString("hex")}`;
  ...
  const currentResult = await db
    ...
    .order("position", { ascending: true });
  if (currentResult.error) {
    return { ok: false, code: "failed", error: "Could not load sections." };
  }
  const currentRows =
    (currentResult.data as Array<{ section_id: string; position: number }> | null) ??
    [];
  if (currentRows.some((row) => row.section_id === sectionId)) {
    return { ok: false, code: "exists", error: "That section already exists." };
  }

  let insertPosition = currentRows.length;
  if (params.insertAfterSectionId) {
    const afterIndex = currentRows.findIndex(
      (row) => row.section_id === params.insertAfterSectionId,
    );
    if (afterIndex < 0) {
      return { ok: false, code: "not_found", error: "Section not found." };
    }
  ...
  const now = new Date().toISOString();
  const laterRows = currentRows
    .filter((row) => row.position >= insertPosition)
  ...
  for (const row of laterRows) {
    ...
    if (shifted.error) {
      return { ok: false, code: "failed", error: "Could not create the section." };
    }
  }

  const insertResult = await db.from("creed_sections").insert({
...
type PersonalLifecycleFailure = {
  ok: false;
  code: "forbidden" | "not_found" | "failed";
  error: string;
};

async function requireOwnedPersonalCreed(
  db: SupabaseLikeClient,
  userId: string,
  creedId: string,
): Promise<string | PersonalLifecycleFailure> {
  const personalCreedId = await getOwnedPersonalCreedId(db, userId, creedId);
  if (personalCreedId !== creedId) {
    return {
      ok: false,
      code: "forbidden",
      error: "You cannot edit this Creed.",
...
export async function deletePersonalSection(
  client: unknown,
  userId: string,
  params: { creedId: string; sectionId: string },
): Promise<{ ok: true; revision: number } | PersonalLifecycleFailure> {
  const db = client as SupabaseLikeClient;
  const owned = await requireOwnedPersonalCreed(db, userId, params.creedId);
...
export async function setPersonalSectionArchived(
  client: unknown,
  userId: string,
  params: { creedId: string; sectionId: string; archived: boolean },
): Promise<{ ok: true; revision: number } | PersonalLifecycleFailure> {
  const db = client as SupabaseLikeClient;
  const owned = await requireOwnedPersonalCreed(db, userId, params.creedId);
  if (typeof owned !== "string") return owned;
...
export async function setPersonalSectionPermission(
  client: unknown,
  userId: string,
  params: { creedId: string; sectionId: string; agentPermission: AgentPermission },
): Promise<{ ok: true; revision: number } | PersonalLifecycleFailure> {
  const db = client as SupabaseLikeClient;
  const owned = await requireOwnedPersonalCreed(db, userId, params.creedId);
  if (typeof owned !== "string") return owned;
  ...
  const currentResult = await db
    .from("creed_sections")
    .select("revision, payload, agent_permission")
    .eq("creed_id", params.creedId)
    .eq("section_id", params.sectionId)
    .maybeSingle();
  if (currentResult.error) {
  ...
  if (current.agent_permission === params.agentPermission) {
    return { ok: true, revision: current.revision };
  }
  ...
  const updateResult = await db
    .from("creed_sections")
    .update({
      agent_permission: params.agentPermission,
      agent_writable: permissionToWritable(params.agentPermission),
      payload: {
        ...current.payload,
        agentPermission: params.agentPermission,
        agentWritable: permissionToWritable(params.agentPermission),
      },
      updated_at: now,
    })
    .eq("creed_id", params.creedId)
...
export async function reorderPersonalSections(
  client: unknown,
  userId: string,
  params: { creedId: string; sectionIds: string[] },
): Promise<{ ok: true } | PersonalLifecycleFailure> {
  const db = client as SupabaseLikeClient;
  const owned = await requireOwnedPersonalCreed(db, userId, params.creedId);
  if (typeof owned !== "string") return owned;
  if (params.sectionIds.length === 0) {
    return { ok: false, code: "failed", error: "Could not save the new order." };
  }
  ...
  const currentRows =
    (currentResult.data as Array<{ section_id: string; position: number }> | null) ??
    [];
  const known = new Set(currentRows.map((row) => row.section_id));
  if (params.sectionIds.some((id) => !known.has(id))) {
    return { ok: false, code: "not_found", error: "Section not found." };
  }

  const requested = new Set(params.sectionIds);
  const preserved = currentRows
    .filter((row) => !requested.has(row.section_id))
    .map((row) => row.section_id);
  const ordered = [...params.sectionIds, ...preserved];
  const now = new Date().toISOString();
...
async function findCredentialByTokenHash(
  db: SupabaseLikeClient,
  table: "creed_tokens",
  hashColumn: "read_token_hash" | "proposal_token_hash" | "direct_edit_token_hash",
  token: string,
  errorMessage: string,
