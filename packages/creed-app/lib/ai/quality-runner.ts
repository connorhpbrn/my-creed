...
function freezeSnapshot(): RunnerSnapshot {
  ...
  return Object.freeze({
    report,
    fullRunning: [...activeFullRuns].some((key) => key.startsWith(activePrefix)),
    ...
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeoutId = setTimeout(() => {
          reject(new Error("Could not refresh analysis status. Try again shortly."));
        }, timeoutMs);
      }),
    ]);
  } finally {
...
async function waitForQualityRun(run: QualityRun): Promise<QualityRun> {
  ...
  while (current.status === "queued" || current.status === "running") {
    ...
    const result = await requestQualityJson(`/api/app/ai/quality?runId=${encodeURIComponent(current.id)}`, {
      method: "GET",
      cache: "no-store",
