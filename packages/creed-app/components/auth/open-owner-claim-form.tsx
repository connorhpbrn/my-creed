...
import { DelayedNavigationSpinner } from "@/components/marketing/delayed-navigation-spinner";
...
export function OpenOwnerClaimForm({ nextPath = "/file" }: { nextPath?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} aria-busy={submitting || undefined} className="w-auto">
      <h1
        aria-live="polite"
        className="flex h-9 items-center justify-center gap-0 text-[1.5rem] font-medium tracking-[-0.03em] text-[var(--creed-text-primary)]"
      >
        <SwapLabel
          value={title}
          options={["Enter code", "Entering"]}
        />
        <DelayedNavigationSpinner pending={submitting} />
      </h1>
