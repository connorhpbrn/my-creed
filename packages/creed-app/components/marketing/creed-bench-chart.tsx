...
const CHART_SERIES: BenchSeries[] = [];
...
function BenchTooltip({
  ...
  if (selectedModel && point.model !== selectedModel) return null;
...
function BenchPointShape({
  ...
  return (
    <rect
      ...
      fill={fill}
      className={cn(
        "cursor-default transition-transform duration-150 ease-[cubic-bezier(0.22,1,0.36,1)]",
        interactive && "hover:scale-150",
      )}
...
const LEGEND_HIGHLIGHT_TRANSITION = {
  type: "spring",
  stiffness: 420,
  damping: 34,
} as const;
  ...
  return (
    <div>
      <div className="mb-1 flex flex-col gap-4 text-left sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center justify-start gap-x-1 gap-y-1 sm:justify-end">
          {CHART_SERIES.map((series) => {
            const selected = selectedModel === series.model;
            return (
              <button
                key={series.model}
                type="button"
                ...
                onClick={() =>
                  setSelectedModel((current) =>
                    current === series.model ? null : series.model,
                  )
                }
                className={cn(
                  "group relative z-10 inline-flex items-center gap-1.5 rounded-sm px-1.5 py-1 text-[12px] text-[var(--creed-text-secondary)] transition-[color,background-color] duration-150 ease-[cubic-bezier(0.22,1,0.36,1)] hover:bg-[var(--creed-surface-raised)]/30 hover:text-[var(--creed-text-primary)]",
                  selected && "text-[var(--creed-text-primary)] hover:bg-transparent",
                )}
              >
                {selected ? (
                  <motion.span
                    layoutId="bench-legend-highlight"
                    className="absolute inset-0 -z-10 rounded-sm bg-[var(--creed-surface-raised)]"
                    transition={LEGEND_HIGHLIGHT_TRANSITION}
                  />
                ) : null}
      ...
      {CHART_SERIES.length ? (
        <ChartContainer
          ...
          className={cn(
            // Isolate keeps a hovered point's transform inside the series opacity.
            "h-[430px] w-full min-w-0 aspect-auto [&_.recharts-scatter]:isolate [&_.recharts-scatter]:transition-opacity [&_.recharts-scatter]:duration-[280ms] [&_.recharts-scatter]:ease-[cubic-bezier(0.22,1,0.36,1)] [&_.recharts-wrapper]:outline-none [&_.recharts-wrapper_*]:outline-none sm:h-[520px]",
            selectedModel &&
              "[&_.recharts-scatter]:opacity-[0.18] [&_.recharts-scatter.bench-series-focus]:opacity-100",
          )}
        >
          <ScatterChart margin={{ top: 12, right: 40, bottom: 20, left: 4 }}>
            ...
            <ChartTooltip
              ...
              content={<BenchTooltip selectedModel={selectedModel} />}
            />
            {[...CHART_SERIES]
              .sort((a, b) => {
              ...
              const focused = selectedModel === series.model;
              const dimmed = Boolean(selectedModel) && !focused;
              return (
                <Scatter
                  key={series.model}
                  name={series.model}
                  data={data}
                  fill={series.color}
                  className={focused ? "bench-series-focus" : undefined}
                  zIndex={focused ? 650 : 600}
                  line={{ stroke: series.color, strokeWidth: 2 }}
                  lineType="joint"
                  shape={<BenchPointShape interactive={!dimmed} />}
                  isAnimationActive
