...
import { MarketingRouteTransition } from "@/components/marketing/marketing-route-transition";
...
import {
  DelayedNavigationSpinner,
  useDelayedNavigationPending,
} from "@/components/marketing/delayed-navigation-spinner";
...
function editionNavGroups(hasHostedAccounts: boolean) {
  return navGroups.map((group) => ({
    ...
    items: group.items.filter((item) => {
      if (item.href === "/sponsor" && !hasHostedAccounts) return false;
      if (item.href === "/bench" && !BENCH_PAGE_PUBLIC) return false;
      return true;
    }),
  }));
}
...
// Shared framed hero image for the inner marketing pages.
export function MarketingHeroBanner({
  ...
  return (
    <section className="relative bg-[var(--creed-background)]">
      <div data-marketing-hero className="relative mx-2 mt-2 h-[22rem] overflow-hidden rounded-[20px] md:h-[24rem]">
...
export function MarketingHeader({
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      setMobileMenuSurfaceHeight(menu.offsetTop + menu.offsetHeight + 8);
    };
      ...
      >
        ...
        <header
          className={cn(
            ...
            stickyChromeActive
              ? "py-1.5 pl-2.5 pr-1.5 md:pl-4"
              : "py-1.5 pl-2.5 pr-1.5 md:px-1",
          )}
        >
...
function GitHubStarButton({
  ...
  return (
    <a
      ...
      onClick={onNavigate}
      className={cn(
        "github-star-button inline-flex h-8 items-center gap-2.5 rounded-md border px-2.5 text-[14px] font-medium shadow-none transition-colors duration-300",
        scrolled
          ? "border-white/15 bg-[var(--creed-accent)] text-white hover:bg-[var(--creed-accent-hover)]"
          : "border-transparent bg-white text-[#19345f] hover:bg-[#f6f7fb]",
        className,
      )}
    >
...
// the same style as the mobile nav. Used for the centre navigation groups and
// closed on outside click, scroll, or Escape.
...
function HeaderAccountLink({
  ...
  return (
    <Link
      href={signedIn ? continueHref : "/pricing"}
      aria-busy={pending}
      onClick={(event) => {
        beginPending(event);
        onNavigate?.();
      ...
      className={cn(
        "gap-0! overflow-hidden rounded-md transition-colors duration-200",
        ...
        scrolled
          ...
          : "h-8 items-center px-2.5 text-[14px] font-medium text-white hover:text-white/55",
      )}
    >
      {signedIn ? "Continue" : "Start"}
      <DelayedNavigationSpinner pending={pending} />
    </Link>
  );
}
...
export function MarketingFooter() {
  ...
  return (
    <footer className="border-t border-[var(--creed-border)] px-6 pt-8 md:px-10 lg:px-12">
      ...
      <div className="mx-auto mt-8 flex max-w-7xl flex-col gap-4 border-t border-[var(--creed-border)] pt-8 pb-6 md:flex-row md:items-center md:justify-between">
        <BrandedCredit
