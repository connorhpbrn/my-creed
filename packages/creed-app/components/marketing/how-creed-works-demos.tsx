...
const ACTIVITY_STEPS = [1200, 1100, 1700, 2200, 1700, 800] as const;
const ACTIVITY_PRESS_MS = 170;
const ACTIVITY_SIDEBAR_DELAY_MS = 240;
...
export function ActivityDemo() {
  ...
  // Press plays only on open, then the sidebar follows. Close skips the press
  // so the panel just slides away.
  useEffect(() => {
    if (step !== 1) return;
    setPressed(true);
    const id = window.setTimeout(() => setPressed(false), ACTIVITY_PRESS_MS);
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => setPanelOpen(true), ACTIVITY_SIDEBAR_DELAY_MS);
    return () => window.clearTimeout(id);
  }, [wantPanel]);
    ...
    <div ref={ref} className="w-full">
      <DemoCard className="relative h-[352px] overflow-hidden p-0">
        <div className="flex h-11 items-center border-b border-[var(--creed-border)] px-3">
          ...
          <button
            type="button"
            onClick={() => setStep(wantPanel ? 5 : 1)}
            className={cn(
              "ml-auto inline-flex items-center gap-1 rounded-[6px] border border-[var(--creed-border)] bg-[var(--creed-surface-raised)] px-1.5 py-0.5 text-[10px] font-medium text-[var(--creed-text-secondary)] transition-all duration-150",
              pressed &&
      ...
      <DemoCard>
        ...
        <div className="relative mt-3 border-t border-[var(--creed-border)] pt-2">
          <motion.div
            ...
            className="pointer-events-none absolute left-1.5 top-3 text-[12px] text-[var(--creed-text-tertiary)]"
          >
          ...
          <div
            className={cn(
              ...
              resolved ? "grid-rows-[1fr]" : "grid-rows-[0fr]",
            )}
          >
