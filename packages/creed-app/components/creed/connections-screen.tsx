...
function agentsFilterEmptyLabel(type: string, status: string) {
  const typeLabel =
    type === "all"
      ? null
      : (AGENT_CATEGORY_FILTER_ITEMS.find((item) => item.key === type)?.label ??
        null);
  if (status === "all") {
    return typeLabel ? `No ${typeLabel} agents yet.` : "No agents yet.";
  }
  if (status === "not-connected") {
    return typeLabel
      ? `No ${typeLabel} agents that aren't connected.`
      : "No agents that aren't connected.";
  }
  const statusPhrase = status === "disconnected" ? "disconnected" : "connected";
  return typeLabel
    ? `No ${statusPhrase} ${typeLabel} agents.`
    : `No ${statusPhrase} agents.`;
}

function AgentsFilterEmpty({
  type,
  status,
}: {
  type: string;
  ...
  const { iconRef, start, settle } = useAnimatedIconControls(120, undefined, 1800);
  return (
    <div
    ...
    >
      ...
      <span className="font-medium opacity-60">
        {agentsFilterEmptyLabel(type, status)}
      </span>
    </div>
  );
}
...
export function ConnectionsScreen({ active = true }: { active?: boolean }) {
  ...
  useEffect(() => {
    ...
    void fetch(
      `/api/app/mcp/live?creedId=${encodeURIComponent(creedId)}`,
    )
      .then(async (response) => {
      ...
      <div className="mx-auto max-w-[960px] overflow-hidden px-4 py-8 md:px-12 md:py-10">
        ...
        <div className="mt-5 flex h-auto flex-col rounded-xl border border-[var(--creed-border)] bg-[var(--creed-surface)] p-4 md:p-5">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            >
              <SwapLabel
                value={setupOpen ? "Hide instructions" : "Show instructions"}
                options={["Show instructions", "Hide instructions"]}
              />
            </Button>
          </div>
