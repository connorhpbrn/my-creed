...
import { ShredderIcon } from "@creed/ui/shredder";
...
export function FileScreen({ active = true }: { active?: boolean }) {
    ...
    async function loadVersionStatus() {
      ...
      try {
        ...
        const localHash = Array.from(new Uint8Array(buffer))
          ...
          .join("");
        const response = await fetchForCreed(state.creedId,
  ...
  useEffect(() => {
    ...
    async function loadAiReadiness() {
      try {
        ...
        // Leave the optimistic enabled state so a settings blip does not hide
        // the analysis control before the first run is possible.
      }
    }
  ...
  async function handleOpenPushReview() {
    ...
    try {
      ...
      const response = await fetchForCreed(state.creedId,"/api/app/github/pull/preview", {
        method: "POST",
  ...
  async function handlePushCreed() {
    try {
      ...
      const localHash = Array.from(new Uint8Array(buffer))
        .map((value) => value.toString(16).padStart(2, "0"))
        .join("");
      const response = await fetchForCreed(state.creedId,"/api/app/github/push", {
        method: "POST",
        headers: {
  ...
  async function handleApplyPull() {
    ...
    try {
      setPullBusy(true);
      const response = await fetchForCreed(state.creedId,"/api/app/github/pull/apply", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
                    ...
                    <div ref={composerAreaRef} className="mt-10 md:mt-16">
                      {composerOpen ? (
                        <motion.div
                          ...
                          className={cn(
                            "relative rounded-lg border border-[var(--creed-border)] bg-[var(--creed-surface)] p-4 sm:p-5",
                        ...
                        >
                          ...
                          <Button
                            variant="ghost"
                            size="icon-sm"
                            className="absolute top-2 right-2 rounded-md sm:top-3 sm:right-3"
                            onClick={() => {
                              setComposerOpen(false);
                              setComposerRevealed(false);
                            }}
...
const ActivityRailContent = memo(function ActivityRailContent({
  ...
  const filteredAll = useMemo(
    ...
    [activity, proposals, creedType, livePendingProposalIds, statusFilter],
  );
