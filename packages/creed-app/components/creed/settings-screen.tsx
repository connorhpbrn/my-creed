...
function PersonalSettingsScreen() {
  ...
  useEffect(() => {
    ...
    async function loadRepos() {
      try {
        ...
        const loadedRepos = await loadSettingsRepos(state.creedId);
  ...
  useEffect(() => {
    ...
    async function updateStatus() {
      ...
      try {
        ...
        const status = await loadSettingsVersionStatus(localHash, state.creedId);
  ...
  function handleConnectGitHub() {
    ...
    // eslint-disable-next-line @next/next/no-location-assign-relative-destination
    window.location.href = `/api/app/github/authorize?mode=personal&creedId=${encodeURIComponent(state.creedId ?? "")}`;
  }

  async function handleDisconnectGitHub() {
    try {
      setDisconnectingGitHub(true);
      const response = await fetchForCreed(state.creedId,"/api/app/github/integration", {
        method: "DELETE",
      });
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              {hasManagedCredits ? (
                ...
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <SwapLabel
                      value={aiSettings.aiMode === "credits" ? "Credits" : "BYOK"}
                      options={["Credits", "BYOK"]}
                    />
...
function usageRangeLabel(range: AiUsageRange) {
  return (
    USAGE_RANGE_ITEMS.find((item) => item.key === range)?.label ?? range
  );
}
