...
import { persistenceRequest } from "@creed/core/validation/creed-state";
import { DocumentWriteQueue } from "@/lib/document-write-queue";
import { PersistenceConflict } from "@/lib/persistence-baseline";
import { applyPersonalProposal, restoreProposalDraft } from "@/lib/personal-proposal-review";
...
function mergeExternalState(
  ...
  if (canReplaceSections && pendingSectionIds.size === 0) {
    sections = incoming.sections;
  } else if (canReplaceSections || replaceSharedSections) {
  ...
  const sectionRevisions = canReplaceSections && pendingSectionIds.size === 0
    ? incoming.sectionRevisions
    : canReplaceSections || replaceSharedSections
      ? {
          ...incoming.sectionRevisions,
          ...Object.fromEntries(
  ...
  return {
    ...
    persistenceSettingsBaseline: canReplaceSections ? incoming.persistenceSettingsBaseline : current.persistenceSettingsBaseline,
    persistenceBaseline: canReplaceSections ? {
      ...incoming.persistenceBaseline,
      ...Object.fromEntries(Array.from(pendingSectionIds).filter((id) => current.persistenceBaseline?.[id]).map((id) => [id, current.persistenceBaseline![id]])),
    } : current.persistenceBaseline,
...
export function CreedProvider({
  ...
  const documentWrites = useRef(new DocumentWriteQueue());

  const persistState = useCallback(
    async (nextState: CreedState, keepalive = false) => {
      if (pendingProposalOutcomesRef.current.size) throw new Error("Waiting for the proposal review to finish.");
      if (!persistenceEnabled) {
        return;
      }

      return documentWrites.current.run(nextState.creedId ?? "", async () => {
        const current = latestStateRef.current;
        const savedState = current.creedId === nextState.creedId ? current : nextState;
        const response = await fetch("/api/app/state", {
          method: "PUT", headers: { "Content-Type": "application/json" }, keepalive,
          body: JSON.stringify({ state: persistenceRequest(savedState) }),
        });
        const data = await response.json().catch(() => ({})) as {
          error?: string; persistenceBaseline?: CreedState["persistenceBaseline"];
        };
        if (response.status === 409) throw new PersistenceConflict();
        if (!response.ok) throw new Error(data.error ?? "Could not save Creed.");
        if (!data.persistenceBaseline) throw new Error("The save response was incomplete.");
        if (latestStateRef.current.creedId === savedState.creedId) {
          const acknowledged = {
            ...latestStateRef.current,
            persistenceBaseline: data.persistenceBaseline,
            persistenceSettingsBaseline: { requireApproval: savedState.settings.requireApproval, versionControl: savedState.settings.versionControl },
            sectionRevisions: Object.fromEntries(Object.entries(data.persistenceBaseline).map(([id, baseline]) => [id, baseline.revision])),
          };
          latestStateRef.current = acknowledged;
  ...
  const flushPendingState = useCallback(
    async (snapshot: CreedState, keepalive = false) => {
      ...
      try {
        const savedState = await persistState(snapshot, keepalive);
        ...
        lastPersistedTickRef.current = savedState.mutationTick;
        broadcastStateChanged();
        setState((current) => current.mutationTick === savedState.mutationTick
          ? { ...current, saving: false, lastSavedAt: Date.now() } : current);
        if (
          latestStateRef.current.mutationTick === savedState.mutationTick &&
  ...
  // Persist an optimistically-added section. The section already lives
  // in local state; success records its revision and baseline so later
  // full-state writes still match the database. A failure toasts and re-syncs.
  async function runSharedSectionCreate(section: CreedSection, afterSectionId?: string) {
    ...
    try {
      const res = await documentWrites.current.run(creedId, () =>
        fetch("/api/app/sections", {
          method: "POST",
      ...
      const position =
        typeof data.position === "number"
          ? data.position
          : latestStateRef.current.sections.findIndex(
      ...
      setState((s) => ({
        ...
        persistenceBaseline: {
          ...s.persistenceBaseline,
          [section.id]: {
            revision,
            position: position < 0 ? s.sections.length - 1 : position,
            permission: section.agentPermission ?? "propose",
            archived: false,
          },
        },
      }));
  ...
  // Name and accent use the same per-section PUT as content so personal
  // Creeds do not send a full-state write through commit_personal_creed.
  async function saveSharedSectionMeta(
    ...
    const creedId = latestStateRef.current.creedId;
    if (!creedId) return;
    await documentWrites.current.run(creedId, async () => {
      ...
      try {
        ...
        const { response, data } = await put(baseRevision);
        if (!response.ok) {
          if (response.status === 409) {
            toast.error("This section changed elsewhere. Your local draft is retained.");
            return;
          }
        ...
        setState((s) => {
          const acknowledged = {
            ...
            persistenceBaseline: s.persistenceBaseline?.[sectionId]
              ? {
                  ...s.persistenceBaseline,
                  [sectionId]: {
                    ...s.persistenceBaseline[sectionId],
                    revision:
                      data.revision ?? s.persistenceBaseline[sectionId].revision,
                  },
                }
              : s.persistenceBaseline,
            sectionRevisions: {
          ...
          if (latestStateRef.current.creedId === creedId) {
            latestStateRef.current = acknowledged;
          }
          return acknowledged;
        });
  ...
  async function runSharedSectionPermission(
    sectionId: string,
    ...
    try {
      const response = await fetch(
        `/api/app/sections/${encodeURIComponent(sectionId)}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ creedId, agentPermission }),
        },
      );
      if (!response.ok) {
        const data = (await response.json().catch(() => ({}))) as {
        ...
        toast.error(data.error ?? "Could not update the section.");
        await syncFromServer();
      }
    } catch {
  ...
  async function runSharedSectionDelete(sectionId: string): Promise<boolean> {
    const creedId = latestStateRef.current.creedId;
  ...
  // Serialize local edits; a remote revision conflict must preserve the draft.
  const sectionSavePromises = useRef(new Map<string, Promise<void>>());
  function runSharedSave(sectionId: string): Promise<void> {
    const running = sectionSavePromises.current.get(sectionId);
    if (running) {
      sharedSaveDirty.current.add(sectionId);
      return running;
    }
    const targetCreedId = latestStateRef.current.creedId;
    const task = documentWrites.current.run(targetCreedId ?? "", async () => {
      if (latestStateRef.current.creedId === targetCreedId) await performSectionSave(sectionId);
    });
    sectionSavePromises.current.set(sectionId, task);
    void task.finally(() => {
      if (sectionSavePromises.current.get(sectionId) === task) sectionSavePromises.current.delete(sectionId);
    });
    return task;
  }

  async function performSectionSave(sectionId: string) {
    const creedId = latestStateRef.current.creedId;
    if (!creedId) return;
    const saveGeneration = activeCreedGenerationRef.current;
    const isCurrentCreed = () =>
    ...
    try {
      ...
      const acknowledged = {
        ...
        persistenceBaseline: current.persistenceBaseline?.[sectionId] ? {
          ...current.persistenceBaseline,
          [sectionId]: { ...current.persistenceBaseline[sectionId], revision: data.revision ?? current.persistenceBaseline[sectionId].revision },
        } : current.persistenceBaseline,
        sectionRevisions: { ...current.sectionRevisions, [sectionId]: data.revision ?? current.sectionRevisions[sectionId] ?? 1 },
      };
      latestStateRef.current = acknowledged;
      setState(acknowledged);
      offlineRetrySectionsRef.current.delete(sectionId);
      ...
      if (isCurrentCreed()) {
        ...
        if (saveSucceeded && sharedSaveDirty.current.has(sectionId)) {
          sharedSaveDirty.current.delete(sectionId);
          sectionSavePromises.current.delete(sectionId);
          void runSharedSave(sectionId);
        } else if (
          sharedSaveInFlight.current.size === 0 &&
          sharedSaveTimers.current.size === 0 &&
  ...
  const syncFromServer = useCallback(async () => {
    ...
    try {
      ...
      response = await fetch(
        ...
        {
        method: "GET",
        headers: { "x-creed-id": latestStateRef.current.creedId ?? "" },
        cache: "no-store",
        },
      );
    } catch {
    ...
    setState((current) => {
      ...
      const canReplaceSections =
        pendingProposalOutcomesRef.current.size === 0 && current.mutationTick === lastPersistedTickRef.current;
  ...
  const switchCreed = useCallback(
    async (creedId: string): Promise<{ ok: boolean; error?: string }> => {
      if (pendingProposalOutcomesRef.current.size) return { ok: false, error: "Wait for the proposal review to finish before switching Creed." };
      const leavingId = latestStateRef.current.creedId;
      const pendingSections = new Set([
        ...sharedSaveTimers.current.keys(), ...sectionSavePromises.current.keys(), ...sharedSaveInFlight.current,
        ...sharedSaveDirty.current, ...offlineRetrySectionsRef.current,
      ]);
      for (const timer of sharedSaveTimers.current.values()) window.clearTimeout(timer);
      sharedSaveTimers.current.clear();
      await Promise.all(Array.from(pendingSections, (id) => runSharedSave(id)));
      if (sharedSaveDirty.current.size || sharedSaveInFlight.current.size || offlineRetrySectionsRef.current.size) {
        return { ok: false, error: "Your changes have not saved yet. Resolve the save before switching Creed." };
      }
      if (saveTimerRef.current !== null) {
        window.clearTimeout(saveTimerRef.current);
        saveTimerRef.current = null;
      }
      if (persistenceEnabled && latestStateRef.current.mutationTick !== lastPersistedTickRef.current) {
        await flushPendingState(latestStateRef.current);
        if (latestStateRef.current.mutationTick !== lastPersistedTickRef.current) {
          return { ok: false, error: "Your changes have not saved yet." };
        }
      }
      if (leavingId) await documentWrites.current.drain(leavingId);
  ...
  function toggleLock() {
    // Lock is session-only. Bumping mutationTick would send a full-file
    // personal save that does not persist lock state.
    commitState((current) => ({
      ...current,
  ...
  function updateRichTextSection(sectionId: string, content: string) {
    ...
    // Text uses the per-section save path. Leave mutationTick unchanged so
    // typing does not trigger a full-state write.
    commitState((current) =>
      ({
        ...current,
  ...
  function renameSection(sectionId: string, name: string) {
    ...
    // Name uses the per-section PUT, not a full-state write. Bumping
    // mutationTick here would send personal Creeds through commit_personal_creed.
    commitState((current) =>
      ({
        ...current,
        sections: current.sections.map((section) =>
  ...
  function duplicateSection(sectionId: string) {
    ...
    const clone: CreedSection = {
      ...cloneSection(section),
      id:
        snapshot.creedType === "shared"
          ? newSharedSectionId()
          : uniqueLocalId("section"),
    };
    ...
    void runSharedSectionCreate(clone, sectionId);
  }
  ...
  function deleteSection(sectionId: string) {
    ...
    void runSharedSectionDelete(sectionId);
  }
  ...
  function archiveCreed() {
    const liveIds = latestStateRef.current.sections
      .filter((section) => !section.archived)
      .map((section) => section.id);
    const placeholder: CreedSection | null =
      latestStateRef.current.creedType === "shared"
        ? null
        : {
    ...
    void Promise.all(
      liveIds.map((sectionId) =>
        runSharedSectionArchive(sectionId, true, { sync: false }),
      ),
    ).then(async () => {
      if (placeholder) await runSharedSectionCreate(placeholder);
      await syncFromServer();
    });
  }
  ...
  function clearSections() {
    ...
    const previousIds = latestStateRef.current.sections.map(
      (section) => section.id,
    );
    ...
    void (async () => {
      for (const sectionId of previousIds) {
        const deleted = await runSharedSectionDelete(sectionId);
        if (!deleted) return;
      }
      for (const section of defaultSections) {
        await runSharedSectionCreate(section);
      }
    })();
  }
  ...
  async function reviewSharedProposalRemote(
    ...
    const creedId = latestStateRef.current.creedId;
    if (!creedId) return;
    const generation = activeCreedGenerationRef.current;
    pendingProposalOutcomesRef.current.set(proposalId, "removed");
    try {
      const response = await documentWrites.current.run(creedId, () =>
        fetch(`/api/app/proposals/${encodeURIComponent(proposalId)}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ creedId, decision }),
        }),
      );
      if (generation !== activeCreedGenerationRef.current) return;
      if (!response.ok) {
        const data = (await response.json().catch(() => ({}))) as {
          error?: string;
          code?: string;
  ...
  async function reviewPersonalProposalRemote(
    ...
    const proposal = before.proposals.find((entry) => entry.id === proposalId);
    if (!proposal) return;
    if (pendingProposalOutcomesRef.current.size || sectionSavePromises.current.size || sharedSaveTimers.current.size || sharedSaveInFlight.current.size || sharedSaveDirty.current.size ||
      before.mutationTick !== lastPersistedTickRef.current) {
      toast.error("Save your current changes before reviewing this proposal.");
      return;
    }
    if (decision === "accept" && proposal.draft.kind !== "new-section" && proposal.baseRevision != null &&
      proposal.baseRevision !== before.sectionRevisions[proposal.sectionId]) decision = "stale";
    let optimistic = before;
    try {
      const next = decision === "accept" ? applyPersonalProposal(before, proposal) : {
        ...before, proposals: decision === "stale"
          ? before.proposals.map((entry) => entry.id === proposalId ? { ...entry, status: "stale" as const } : entry)
          : before.proposals.filter((entry) => entry.id !== proposalId),
      };
      pendingProposalOutcomesRef.current.set(proposalId, decision === "stale" ? "stale" : "removed");
      optimistic = next;
      latestStateRef.current = next;
      setState(next);
      const response = await documentWrites.current.run(creedId, () => fetch(`/api/app/proposals/${encodeURIComponent(proposalId)}`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ creedId, decision }),
      }));
      if (!response.ok) {
        const data = await response.json().catch(() => ({})) as { error?: string };
        throw new Error(data.error ?? "Could not review the proposal.");
      }
      broadcastStateChanged();
    } catch (error) {
      if (generation !== activeCreedGenerationRef.current) return;
      toast.error(error instanceof Error ? error.message : "Could not review the proposal.");
      setState((current) => {
        const restored = restoreProposalDraft(before, optimistic, current, proposal);
        latestStateRef.current = restored;
        return restored;
      });
    } finally {
      if (generation === activeCreedGenerationRef.current) {
        pendingProposalOutcomesRef.current.delete(proposalId);
        stateTickRef.current = null;
        lastSyncAtRef.current = 0;
        await syncFromServer();
  ...
  function acceptProposals(proposalIds: string[]) {
    ...
    void (async () => {
      for (const id of proposalIds) await reviewPersonalProposalRemote(id, "accept");
    })();
  }
  ...
  function setSectionPermission(
    ...
    void runSharedSectionPermission(sectionId, permission);
  }
  ...
  function setAllSectionPermissions(
    ...
    for (const section of latestStateRef.current.sections) {
      if (section.agentPermission === "hidden") continue;
      void runSharedSectionPermission(section.id, permission);
    }
  }
