...
export function CreedSwitcher() {
  ...
  const headerKind =
    (optimisticCreed?.type ?? state.creedType) === "shared" ? "shared" : "person";
      ...
      <DropdownMenu>
        ...
        <DropdownMenuTrigger asChild>
          <button
            ...
            className="group/switcher inline-flex min-w-0 max-w-full items-center gap-2.5 text-left transition-opacity duration-[160ms] hover:opacity-60 disabled:opacity-70"
          >
            <ProfileAvatar
              kind={headerKind}
              name={displayName}
            ...
            <ChevronDown
              className={cn(
                DROPDOWN_CHEVRON_CLASS,
                "text-[var(--creed-text-primary)]! group-data-[state=open]/switcher:rotate-180",
              )}
              strokeWidth={2}
            />
          </button>
