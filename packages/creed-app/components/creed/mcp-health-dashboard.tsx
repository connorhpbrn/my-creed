...
export function McpHealthDashboard({ active = true }: { active?: boolean }) {
  ...
  const chartFilterKey = `${range}-${agentFilter}-${categoryFilter}-${metric}`;
  const panelFilterKey = `${range}-${agentFilter}-${categoryFilter}`;
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            <div className="min-w-0 rounded-xl border border-[var(--creed-border)] bg-[var(--creed-surface)] p-5">
              ...
              <ChartStage
                ...
                loading={<SkeletonBar className="h-[200px] w-full rounded-lg" />}
                  ...
                  >
                    {sections.map((section, index) => (
                      ...
                      >
                        ...
                        <span className="ml-auto shrink-0 font-mono text-[var(--creed-text-primary)]">
                          {section.count}
                        </span>
                      </div>
                    ))}
