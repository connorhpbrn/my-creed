...
export function CreedShell({
  ...
  // Pathname lags because the app layout is force-dynamic. Keep the raised
  // pill on the click target until that same navigation transition finishes.
        ...
        >
          <div className="flex h-full flex-col">
            ...
            <nav className={cn("mt-5 space-y-1", !collapsed && "lg:mt-8")}>
              ...
              {navItems.map((item) => (
                <ShellNavLink
                  key={item.href}
                  item={item}
                  active={optimisticPath === item.href}
                  collapsed={collapsed}
                  onNavigate={() => {
                    startTransition(() => {
            ...
            >
              ...
              >
                <span className="inline-flex h-3.5 w-3.5 shrink-0 items-center justify-center leading-none">
