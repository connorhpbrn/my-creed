...
// Same spring as the live panel: CSS height snaps, this keeps momentum.
...
const PANEL_DEMO_STEPS: PanelDemoStep[] = [
  { mode: "search", view: "type", prompt: "billing" },
  { mode: "search", view: "results", prompt: "billing", holdMs: 3600 },
  { mode: "ask", view: "type", prompt: "what changed in Goals?" },
  { mode: "ask", view: "thinking", prompt: "what changed in Goals?", holdMs: 2000 },
  { mode: "ask", view: "answer", prompt: "what changed in Goals?", holdMs: 4000 },
  ...
  { mode: "agent", view: "result", prompt: "tighten my Work section", holdMs: 4000 },
];
...
function PanelModeIcon({ mode }: { mode: PanelMode }) {
  const Icon = mode === "agent" ? CpuIcon : mode === "ask" ? CompassIcon : SearchIcon;
  return (
    <Icon
      size={16}
...
function panelBodyKey(step: PanelDemoStep) {
  ...
  if (step.mode === "ask") return step.view === "type" ? "ask-idle" : "ask-thread";
...
function PanelSearchBody({ query }: { query: string }) {
  const groups = useMemo(() => {
    ...
    // Empty query keeps Billing on screen. The live panel would list every
    ...
    return [{ label: "Actions" as const, items }];
  }, [query]);
  ...
  return (
    <div className="space-y-2.5 p-2.5">
      <div className="flex items-center gap-2 px-0.5 py-1 text-[13px] text-[var(--creed-text-primary)]">
        ...
        <span className="truncate">Proposed a tighter Work section.</span>
      </div>
      <div className="flex items-center gap-2 px-0.5 py-1 text-[13px] text-[var(--creed-text-secondary)]">
        <Check
  ...
  const inputValue =
    step.view === "type"
      ...
      : step.mode === "search"
        ? step.prompt
        : "";
  const inputKey = inputValue ? `value-${step.mode}` : `placeholder-${step.mode}`;

  return (
    <div
      aria-hidden="true"
      className="pointer-events-none mx-auto w-full max-w-[360px] select-none overflow-hidden rounded-[var(--radius-lg)] bg-[var(--creed-surface)] shadow-[0_18px_48px_rgba(28,28,26,0.08)] ring-1 ring-foreground/8"
    >
    ...
    <section className="px-6 py-24 md:px-10 md:py-30 lg:px-12">
      ...
      <div className="mx-auto mt-14 grid max-w-6xl items-stretch gap-5 lg:grid-cols-3">
        <PlateCard
          ...
          plateClassName="min-h-[272px] lg:min-h-0 [overflow-anchor:none]"
          title="Analysis"
