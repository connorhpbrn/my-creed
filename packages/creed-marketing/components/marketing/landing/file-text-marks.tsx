...
function sectionMarkStyle(accent: AccentKey): CSSProperties & Record<`--${string}`, string> {
...
export function FileCode({ children }: { children: ReactNode }) {
  return (
    <code className="creed-file-code" style={{ borderRadius: TITLE_CODE_RADIUS_PX }}>
      {children}
    </code>
  );
}
...
export function FileHighlight({
  ...
  return (
    <mark
      ...
      style={{ ...sectionMarkStyle(accent), borderRadius: TITLE_MARK_RADIUS_PX }}
    >
      {children}
    </mark>
  );
