...
import { openApiExposesCommitPersonalCreed } from "../lib/personal-write-contract.mjs";
...
export async function checkRemoteReadiness(values, fetchImplementation = fetch) {
  ...
  try {
    ...
    if (typeof schemaVersion !== "string" || schemaVersion < REQUIRED_SCHEMA_VERSION) {
      return { ready: false, reason: "migration" };
    }

    const openApiResponse = await fetchImplementation(
      `${values.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/`,
      {
        headers: {
          apikey: values.SUPABASE_SECRET_KEY,
    ...
    const openApiDocument = await openApiResponse.json().catch(() => null);
    return openApiExposesCommitPersonalCreed(openApiDocument)
      ? { ready: true, schemaVersion }
      : { ready: false, reason: "write-rpc" };
  } catch {
    return { ready: false, reason: "connection" };
  }
}
