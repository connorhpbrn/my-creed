...
test("remote readiness verifies the exact schema version without exposing keys", async () => {
  ...
  const readiness = await checkRemoteReadiness(values, async (url, options) => {
    ...
    if (url.endsWith("/rest/v1/rpc/creed_schema_version")) {
      return new Response(JSON.stringify(REQUIRED_SCHEMA_VERSION), { status: 200 });
    }
    return new Response(
      JSON.stringify({ paths: { "/rpc/commit_personal_creed": { post: {} } } }),
      { status: 200 },
    );
  });
  ...
  assert.equal(requests[2].url.endsWith("/rest/v1/"), true);
  assert.equal(requests[2].options.headers.Accept, "application/openapi+json");
});
...
test("remote readiness fails when PostgREST cannot see the personal write RPC", async () => {
  const values = {
    NEXT_PUBLIC_SITE_URL: "http://localhost:3001",
    NEXT_PUBLIC_SUPABASE_URL: "https://abcdefghijklmnopqrst.supabase.co",
    NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: "sb_publishable_example",
  ...
  const readiness = await checkRemoteReadiness(values, async (url) => {
    if (url.endsWith("/auth/v1/settings")) {
      return new Response("{}", { status: 200 });
    }
    if (url.endsWith("/rest/v1/rpc/creed_schema_version")) {
    ...
    return new Response(
      JSON.stringify({ paths: { "/rpc/creed_schema_version": { post: {} } } }),
      { status: 200 },
    );
  });

  assert.deepEqual(readiness, { ready: false, reason: "write-rpc" });
});
